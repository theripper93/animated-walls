...
class WallAnimation {
  constructor(wall, speed = 1000, options = { type: null, direction: null, distance: null, rotation: null, anchor: null },keyframes = 1000) {
    this.wall = wall;
    this.loop = options.loop;
    this.speed = speed;
    ...
    switch (this.type) {
        case "move":
          ...
          break;
        case "rotate":
        if(this.anchor==this.wall.center)
          this.rotateCenterAnimation();
          else
          this.rotateOuterAnimation();
          break;
    }
  ...
  rotateCenterAnimation() {
    ...
    for(let i = 0; i < this.keyframes; i++){
        ...
        const x1 = center.x + (p1.x - center.x) * Math.cos(angle) - (p1.y - center.y) * Math.sin(angle);
        const y1 = center.y + (p1.x - center.x) * Math.sin(angle) + (p1.y - center.y) * Math.cos(angle);
        const x2 = center.x + (p2.x - center.x) * Math.cos(angle) - (p2.y - center.y) * Math.sin(angle);
        const y2 = center.y + (p2.x - center.x) * Math.sin(angle) + (p2.y - center.y) * Math.cos(angle);
        this.animation.push([x1, y1, x2, y2]);
    }
  ...
  rotateOuterAnimation() {
    ...
    let p = anchor == this.wallPoints.p1 ? this.wallPoints.p2 : this.wallPoints.p1;
    let rotation = this.rotation;
    for(let i = 0; i < this.keyframes; i++){
        const angle = (i / this.keyframes) * rotation;
        const x = anchor.x + (p.x - anchor.x) * Math.cos(angle) - (p.y - anchor.y) * Math.sin(angle);
        const y = anchor.y + (p.x - anchor.x) * Math.sin(angle) + (p.y - anchor.y) * Math.cos(angle);
  ...
  async playAnimation(){
    ...
    for(let frame of this.animation){
        tick = frame;
        canvas.scene.updateEmbeddedDocuments("Wall",[{_id:_this.wall.id,c: frame}]);
